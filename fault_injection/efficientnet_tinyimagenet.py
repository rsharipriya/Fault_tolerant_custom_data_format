"""
unified_ptq_fault_robustness_efficientnet_tinyimagenet.py  [FINAL]
═══════════════════════════════════════════════════════════════════════════════
PTQ-WOQ fault-robustness  —  EfficientNet-B0 / Tiny ImageNet
...
──────────────────────────────────────────────────
  features[0][0] : stride 2 → 1  (preserves spatial resolution on 64×64)
  classifier     : Linear(in_features, 200)
  PTH            : efficientnet_tiny_imagenet.pth

Run once to fix val folder layout:
  python efficientnet_tinyimagenet_final.py --reorganise-val
═══════════════════════════════════════════════════════════════════════════════
"""
...
FMT_COLOR={"E2M13":_T[0],"E3M12":_T[2],"E4M11":_T[4],"E5M10":_T[6],"E6M9":_T[8],
            "E7M8":_T[10],"E8M7":_T[12],"F8E3M4":_T[14],"F8E4M3":_T[16],"F8E5M2":_T[18],
            "INT16":_T[1],"INT8":_T[3],"INT4":_T[5],"INT2":_T[7]}
FMT_LS={**{f:"-" for f in ["E2M13","E3M12","E4M11","E5M10","E6M9","E7M8","E8M7"]},
        **{f:"--" for f in ["F8E3M4","F8E4M3","F8E5M2"]},
        **{f:":" for f in ["INT16","INT8","INT4","INT2"]}}
FMT_MK={**{f:"o" for f in ["E2M13","E3M12","E4M11","E5M10","E6M9","E7M8","E8M7"]},
        **{f:"s" for f in ["F8E3M4","F8E4M3","F8E5M2"]},
        **{f:"^" for f in ["INT16","INT8","INT4","INT2"]}}

# ── Config ────────────────────────────────────────────────────────────────────
DEVICE            = torch.device("cuda" if torch.cuda.is_available() else "cpu")
PTH_PATH          = "efficientnet_tiny_imagenet.pth"
NUM_CLASSES       = 200
DATASET_NAME      = "Tiny ImageNet"
MODEL_NAME        = "EfficientNet-B0"
OUT_PREFIX        = "ptq_unified_efficientnet_tinyimagenet"
...
ACCURACY_DROP_THRESHOLD = 30.0

IMAGENET_MEAN=(0.485,0.456,0.406); IMAGENET_STD=(0.229,0.224,0.225)

FLOAT_FORMATS={"E2M13":(2,13,16),"E3M12":(3,12,16),"E4M11":(4,11,16),
...
FAULT_LABELS={"exp":"Group 1 – Exponent Bits","sign":"Group 2 – Sign Bit",
...
def reorganise_val(val_dir=VAL_DIR):
    ann=os.path.join(val_dir,"val_annotations.txt"); img=os.path.join(val_dir,"images")
    if not os.path.isfile(ann): print("[INFO] val_annotations.txt not found – assuming organised."); return
    if not os.path.isdir(img): print("[INFO] images/ not found – assuming organised."); return
...
def get_loader():
    ...
    tf=transforms.Compose([
        transforms.Resize(72),transforms.CenterCrop(64),
...
def load_model():
    """EfficientNet-B0 for Tiny ImageNet: stride-1 stem, 200-class head."""
    ...
    m.classifier=nn.Sequential(nn.Dropout(p=0.3,inplace=True),nn.Linear(inf,NUM_CLASSES))
...
def plot(fp32_acc,clean_acc,results,skipped):
    ...
    out={"model":MODEL_NAME,"dataset":DATASET_NAME,"num_classes":NUM_CLASSES,"image_size":"64x64",
         "ptq_mode":"weight-only","eval_set":"full_val_10000","n_trials":N_TRIALS,
...
if __name__=="__main__":
    ...
    if args.reorganise_val:
        reorganise_val(VAL_DIR); print("Val split reorganised. Re-run without --reorganise-val.")
    else:
