"""
unified_ptq_fault_robustness_resnet18_tinyimagenet.py  [FINAL]
═══════════════════════════════════════════════════════════════════════════════
Unified PTQ-WOQ fault-robustness  —  ResNet-18 / Tiny ImageNet

KEY CHANGES:
  1. Fault injection uses the FULL 10,000-sample val set (not 1,000 subset)
  2. Early-skip: if a format's clean PTQ accuracy drops more than
     ...
     is skipped entirely. Format still shown in bar chart (hatched).

Model adaptations for 64×64 Tiny ImageNet images
──────────────────────────────────────────────────
  conv1   : 7×7 stride-2 pad-3  →  3×3 stride-1 pad-1
  maxpool : removed (Identity)
  fc      : Linear(512, 200)
  PTH     : resnet18_tiny_imagenet.pth

Run once to fix val folder layout:
  python resnet18_tinyimagenet_final.py --reorganise-val
═══════════════════════════════════════════════════════════════════════════════
"""

import argparse, copy, json, math, os, shutil
import numpy as np
...
PTH_PATH          = "resnet18_tiny_imagenet.pth"
NUM_CLASSES       = 200
DATASET_NAME      = "Tiny ImageNet"
MODEL_NAME        = "ResNet-18"
OUT_PREFIX        = "ptq_unified_resnet18_tinyimagenet"
...
# ── Val reorganiser ───────────────────────────────────────────────────────────
def reorganise_val(val_dir=VAL_DIR):
    ann = os.path.join(val_dir, "val_annotations.txt")
    img = os.path.join(val_dir, "images")
    if not os.path.isfile(ann):
        print("[INFO] val_annotations.txt not found – assuming already organised."); return
    if not os.path.isdir(img):
        print("[INFO] images/ not found – assuming already organised."); return
    print(f"[INFO] Reorganising {val_dir} …")
    i2c = {}
    with open(ann) as f:
        for line in f:
            p = line.strip().split("\t")
            if len(p) >= 2: i2c[p[0]] = p[1]
    moved = 0
    for fname, cls in i2c.items():
        src = os.path.join(img, fname)
        if not os.path.isfile(src): continue
        dst = os.path.join(val_dir, cls, "images"); os.makedirs(dst, exist_ok=True)
        shutil.move(src, os.path.join(dst, fname)); moved += 1
    try: os.rmdir(img)
    except OSError: pass
    print(f"[INFO] Moved {moved} images into {len(i2c)} class folders.\n")

def _val_ok(val_dir=VAL_DIR):
    if not os.path.isdir(val_dir): return False
    for e in os.scandir(val_dir):
...
# ── Data & model ──────────────────────────────────────────────────────────────
def get_loader():
    """Single loader — full 10,000-sample val set for all evaluations."""
    if not _val_ok(VAL_DIR):
        print(f"[WARN] val/ not organised. Run: python {__file__} --reorganise-val  first.")
        raise SystemExit(1)
    ...
    ds = ImageFolder(root=VAL_DIR, transform=tf)
...
def run():
    ...
    print(f"Dataset   : {DATASET_NAME}  ({NUM_CLASSES} classes, 64×64)  |  Val : {VAL_DIR}")
    print(f"Device    : {DEVICE}  [{backend}]")
    print(f"Eval set  : FULL val set (10,000 samples)")
    ...
    for fmt in ALL_FMTS:
        ...
        # ── Fault injection — FULL 10k val set ────────────────────────────────
        ...
        for group in FAULT_GROUPS:
            ...
            for nf in N_FAULTS:
                ...
                for _ in range(N_TRIALS):
                    ...
                    accs.append(evaluate(eval_model, s, loader))  # ← full val set
                arr = np.array(accs)
...
def plot(fp32_acc, clean_acc, results, skipped):
    ...
    fig.suptitle(
        f"PTQ WOQ — Fault Robustness  ({MODEL_NAME} / {DATASET_NAME}, Full 10k Val Set)\n"
    ...
    for i, (fmt, v) in enumerate(zip(ALL_FMTS, vals)):
        ...
        ax0.bar(i, v, color=FMT_COLOR[fmt], edgecolor="black", linewidth=0.45,
                width=0.65, zorder=3,
                hatch="////" if is_skipped else "",
                alpha=0.55 if is_skipped else 1.0)
        label_text = f"{v:.1f}\n(skip)" if is_skipped else f"{v:.1f}"
        ax0.text(i, v + 0.12, label_text, ha="center", va="bottom", fontsize=4.5,
    ...
    fmt_handles = [
        Line2D([0],[0], color=FMT_COLOR[f], ls=FMT_LS[f], marker=FMT_MK[f],
               lw=1.0, markersize=3.0, label=f)
        for f in ALL_FMTS if f not in skipped
    ]
    fig.legend(handles=family_lines + [skip_patch] + fmt_handles,
               loc="lower center", ncol=7, bbox_to_anchor=(0.5, -0.12),
               fontsize=6.0, framealpha=0.9, edgecolor="0.75",
               title="Format  (hatched = skipped, no fault injection)", title_fontsize=6)
    plt.tight_layout(rect=[0, 0.10, 1, 1])
    for ext in ("pdf", "png"):
        fn = f"{OUT_PREFIX}.{ext}"
        plt.savefig(fn, **({"dpi": 300} if ext == "png" else {})); print(f"Saved → {fn}")
    plt.show()
    ...
    out = {
        ...
        "image_size": "64x64", "ptq_mode": "weight-only",
        "eval_set": "full_val_10000", "n_trials": N_TRIALS,
        ...
        "results": {fmt: {grp: {str(nf): results[fmt][grp][nf] for nf in N_FAULTS}
                          for grp in FAULT_GROUPS} for fmt in results},
    }
...
if __name__ == "__main__":
    ...
    parser.add_argument("--reorganise-val", action="store_true",
                        help="Reorganise flat Tiny ImageNet val/ into per-class sub-folders.")
    args = parser.parse_args()
    if args.reorganise_val:
        reorganise_val(VAL_DIR)
        print("Val split reorganised. Re-run without --reorganise-val.")
    else:
        if DEVICE.type != "cuda":
            print("[WARN] No CUDA GPU – NumPy fallback (very slow with full 10k set).\n")
        print("=" * 72)
        print(f"  PTQ-WOQ  —  {MODEL_NAME} / {DATASET_NAME}  [Full 10k val set]")
