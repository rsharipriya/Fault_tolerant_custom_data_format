"""
unified_ptq_fault_robustness_resnet18_cifar100.py  [FINAL]
═══════════════════════════════════════════════════════════════════════════════
Unified PTQ-WOQ fault-robustness  —  ResNet-18 / CIFAR-100

KEY CHANGES FROM ORIGINAL:
  1. Fault injection uses the FULL 10,000-sample test set (not 1,000 subset)
  2. Early-skip: if a format's clean PTQ accuracy drops more than
     ACCURACY_DROP_THRESHOLD (30%) below FP32 baseline, fault injection
     is skipped entirely for that format. The format is still shown in
     the bar chart (with hatching) and printed to console.

Model adaptations for 32×32 CIFAR images
──────────────────────────────────────────
  conv1   : 7×7 stride-2 pad-3  →  3×3 stride-1 pad-1
  maxpool : stride-2 pooling    →  nn.Identity()
  fc      : Linear(512, 100)
  PTH     : resnet18_cifar100.pth
═══════════════════════════════════════════════════════════════════════════════
"""
...
PTH_PATH     = "resnet18_cifar100.pth"
NUM_CLASSES  = 100
DATASET_NAME = "CIFAR-100"
MODEL_NAME   = "ResNet-18"
OUT_PREFIX   = "ptq_unified_resnet18_cifar100"
BATCH_SIZE   = 256
N_TRIALS     = 100
N_FAULTS     = list(range(1, 9))

# ── EARLY-SKIP THRESHOLD ──────────────────────────────────────────────────────
# If (fp32_acc - clean_acc) > this value, skip fault injection for that format.
# Example: fp32=90%, threshold=30% → skip any format with clean_acc < 60%
ACCURACY_DROP_THRESHOLD = 30.0   # percentage points
...
def _get_pu(E, M):
    ...
    if k not in _pu_cache:
        ...
        mod = RawModule(code=_PU, options=(f"-DE_BITS={E}", f"-DM_BITS={M}"),
                        name_expressions=["pack_fp32", "unpack_fp32"])
        _pu_cache[k] = {"pack": mod.get_function("pack_fp32"),
                        "unpack": mod.get_function("unpack_fp32")}
    return _pu_cache[k]
...
def _get_inj():
    ...
    if _inj_fn is None:
        if not CUPY_OK: raise RuntimeError("CuPy unavailable.")
        _inj_fn = RawModule(code=_INJ, options=(),
                            name_expressions=["inject_xor"]).get_function("inject_xor")
    return _inj_fn
...
class FloatStore:
    def __init__(self, model, E, M):
        self.E, self.M = E, M
        self.meta  = [p.data.shape for p in model.parameters()]
        self.sizes = [p.numel()    for p in model.parameters()]
        flat = np.concatenate([p.data.cpu().numpy().ravel()
        ...
        if CUPY_OK and DEVICE.type == "cuda":
            ...
            self._gpu = cp.empty(n, dtype=cp.uint16)
            _run(kk["pack"], n, src, self._gpu, np.int32(n)); self._cpu = None
        else:
            self._gpu = None; self._cpu = _np_pack(flat, E, M)

    def clone(self):
        c = object.__new__(FloatStore)
        c.E,c.M,c.meta,c.sizes = self.E,self.M,self.meta,self.sizes
        c._gpu = self._gpu.copy() if self._gpu is not None else None
        c._cpu = None if self._gpu is not None else self._cpu.copy()
        return c
    ...
    def inject(self, bp, nf):
        ...
        if self._gpu is not None:
            ...
            _run(_get_inj(), nf, self._gpu,
                 cp.array(np.random.randint(0, tot, nf), dtype=cp.int32),
                 cp.array(masks, dtype=cp.uint16), np.int32(nf), np.int32(tot))
        else:
            for idx, mk in zip(np.random.randint(0, self._cpu.size, nf), masks):
                self._cpu[idx] ^= mk

    def apply(self, model):
        if self._gpu is not None:
            kk = _get_pu(self.E, self.M); n = int(self._gpu.size)
            dst = cp.empty(n, dtype=cp.float32)
            _run(kk["unpack"], n, self._gpu, dst, np.int32(n))
...
class IntStore:
    ...
    def clone(self):
        c = object.__new__(IntStore)
        c.bits,c.meta,c.sizes = self.bits,self.meta,self.sizes
        c.scales = self.scales; c._pkd = [a.copy() for a in self._pkd]
        c._cumsz = self._cumsz; return c
...
def run():
    ...
    print(f"Model     : {MODEL_NAME}  [3×3 conv1 s1, Identity maxpool, {NUM_CLASSES}-class fc]")
    ...
    print(f"Skip rule : formats with PTQ accuracy drop > {ACCURACY_DROP_THRESHOLD}% "
          f"below FP32 are shown in bar chart only (no fault injection)")
    print("═" * 72)
    ...
    def _plain(m, ld):
        ...
        for imgs, labels in ld:
            imgs, labels = imgs.float().to(DEVICE, non_blocking=True), \
    ...
    print(f"Skip if clean < {skip_threshold:.2f}%  "
          f"(i.e. PTQ drop > {ACCURACY_DROP_THRESHOLD}%)\n")

    results   = {}          # only populated for non-skipped formats
    clean_acc = {}
    skipped   = []          # formats that were skipped
    ...
    for fmt in ALL_FMTS:
        ...
        clean_acc[fmt] = ca
        drop_from_fp32 = fp32_acc - ca

        # ── EARLY-SKIP CHECK ──────────────────────────────────────────────────
        if drop_from_fp32 > ACCURACY_DROP_THRESHOLD:
            skipped.append(fmt)
            print(f"[{fmt:>8s}]  clean={ca:.2f}%  "
                  f"(FP32 drop={drop_from_fp32:.2f}%)  "
                  f"*** SKIPPED — drop exceeds {ACCURACY_DROP_THRESHOLD}% threshold ***")
            continue    # ← no fault injection for this format

        # ── Fault injection ───────────────────────────────────────────────────
        results[fmt] = {g: {} for g in FAULT_GROUPS}
        eval_model = copy.deepcopy(base)  # reused across all trials; apply() fully overwrites
        print(f"[{fmt:>8s}]  clean={ca:.2f}%  (FP32 drop={drop_from_fp32:.2f}%)")

        for group in FAULT_GROUPS:
            bp = bpg[group]
            print(f"           {FAULT_LABELS[group]:45s}  valid_bits={len(bp):2d}  ",
                  end="", flush=True)
            for nf in N_FAULTS:
                accs = []
                for _ in range(N_TRIALS):
                    s = store_clean.clone(); s.inject(bp, nf)
                    accs.append(evaluate(eval_model, s, loader))
                arr = np.array(accs)
                results[fmt][group][nf] = {"mean": float(arr.mean()), "std": float(arr.std())}
            d1 = ca - results[fmt][group][1]["mean"]
            d8 = ca - results[fmt][group][N_FAULTS[-1]]["mean"]
            print(f"drop@1={d1:.2f}%   drop@8={d8:.2f}%")
        print()

    # ── Summary of skipped formats ────────────────────────────────────────────
    if skipped:
        print("─" * 72)
        print(f"SKIPPED formats (PTQ drop > {ACCURACY_DROP_THRESHOLD}% from FP32={fp32_acc:.2f}%):")
        for fmt in skipped:
            print(f"  {fmt:>8s}  clean={clean_acc[fmt]:.2f}%  "
                  f"drop={fp32_acc - clean_acc[fmt]:.2f}%")
        print("─" * 72 + "\n")
...
def _dd(ax, group, lbl, yl):
    """Plot drop curves only for non-skipped formats."""
    active = [f for f in ALL_FMTS if f not in _SK]
    for fmt in active:
        ca  = _CA[fmt]
        ys  = np.array([ca - _RS[fmt][group][nf]["mean"] for nf in N_FAULTS])
        err = np.array([_RS[fmt][group][nf]["std"]        for nf in N_FAULTS])
        ax.plot(N_FAULTS, ys, color=FMT_COLOR[fmt], ls=FMT_LS[fmt],
                marker=FMT_MK[fmt], zorder=3, label=fmt)
        ax.fill_between(N_FAULTS, ys - err, ys + err,
                        color=FMT_COLOR[fmt], alpha=0.10, linewidth=0)
    ax.set_title(f"{lbl}  {FAULT_LABELS[group]}", loc="left",
...
def plot(fp32_acc, clean_acc, results, skipped):
    global _CA, _RS, _SK
    _CA, _RS, _SK = clean_acc, results, skipped
    ...
    gs = gridspec.GridSpec(3, 2, figure=fig,
                           height_ratios=[1.0, 1.35, 1.35], hspace=0.52, wspace=0.28)

    # ── (a) Bar chart — ALL formats, skipped ones are hatched ─────────────────
    ax0  = fig.add_subplot(gs[0, :])
    x    = np.arange(len(ALL_FMTS))
    vals = [clean_acc[f] for f in ALL_FMTS]

    for i, (fmt, v) in enumerate(zip(ALL_FMTS, vals)):
        is_skipped = fmt in skipped
        bar = ax0.bar(
            i, v,
            color    = FMT_COLOR[fmt],
            edgecolor= "black",
            ...
            width    = 0.65,
            zorder   = 3,
            hatch    = "////" if is_skipped else "",   # ← hatching for skipped
            alpha    = 0.55   if is_skipped else 1.0,  # ← faded for skipped
        )
        label_text = f"{v:.1f}\n(skip)" if is_skipped else f"{v:.1f}"
        ax0.text(i, v + 0.12, label_text,
                 ha="center", va="bottom", fontsize=4.5,
                 color="red" if is_skipped else "black")

    ax0.axhline(fp32_acc, color="#CC0000", ls="--", lw=1.0, zorder=4,
                label=f"FP32 ({fp32_acc:.1f}%)")
    ax0.axhline(fp32_acc - ACCURACY_DROP_THRESHOLD,
                color="orange", ls=":", lw=1.0, zorder=4,
                label=f"Skip threshold ({fp32_acc - ACCURACY_DROP_THRESHOLD:.1f}%)")

    ax0.set_xticks(x); ax0.set_xticklabels(ALL_FMTS, rotation=32, ha="right")
    ...
    ax0.set_ylim(max(0, min(vals) - 6), min(100, max(vals) + 3))
    ...
    # ── (b)–(e) Fault-drop plots — only non-skipped formats ───────────────────
    ...
    # ── Legend ────────────────────────────────────────────────────────────────
    family_lines = [
        Line2D([],[],color="k",ls="-", lw=1.0,label="─── Custom Float-16"),
        Line2D([],[],color="k",ls="--",lw=1.0,label="--- Custom Float-8"),
        Line2D([],[],color="k",ls=":", lw=1.0,label="··· INT"),
    ]
    skip_patch = Patch(facecolor="grey", hatch="////", alpha=0.5,
                       edgecolor="black", label=f"Skipped (drop>{ACCURACY_DROP_THRESHOLD}%)")
    fmt_handles = [
        Line2D([0],[0], color=FMT_COLOR[f], ls=FMT_LS[f],
               marker=FMT_MK[f], lw=1.0, markersize=3.0, label=f)
        for f in ALL_FMTS if f not in skipped
    ]
    fig.legend(
        handles=family_lines + [skip_patch] + fmt_handles,
        loc="lower center", ncol=7,
        bbox_to_anchor=(0.5, -0.12),
        fontsize=6.0, framealpha=0.9, edgecolor="0.75",
        title="Format  (hatched = skipped, no fault injection)",
        title_fontsize=6,
    )
    plt.tight_layout(rect=[0, 0.10, 1, 1])
    for ext in ("pdf", "png"):
        fn = f"{OUT_PREFIX}.{ext}"
        plt.savefig(fn, **({"dpi": 300} if ext == "png" else {}))
        print(f"Saved → {fn}")
    plt.show()

    # ── Heatmap — only non-skipped formats ────────────────────────────────────
    active = [f for f in ALL_FMTS if f not in skipped]
    if active:
        nf8 = N_FAULTS[-1]
        dm  = np.array([[clean_acc[fmt] - results[fmt][grp][nf8]["mean"]
                         for grp in FAULT_GROUPS] for fmt in active])
        fig2, ax2 = plt.subplots(figsize=(6, max(4, len(active) * 0.55)))
        vmax = max(dm.max(), 1.0)
        ...
        ax2.set_xticklabels(["Exponent Bits","Sign Bit",
                              "Mantissa / Value\nMSB (top 50%)",
                              "Mantissa / Value\nLSB (bottom 50%)"],
                             fontsize=6.5, ha="center")
        ax2.set_yticks(range(len(active))); ax2.set_yticklabels(active, fontsize=6.5)
        ax2.set_title(
            f"Accuracy Drop Heatmap @ {nf8} Bit-Flips\n"
            f"{MODEL_NAME} / {DATASET_NAME}  (skipped formats excluded)",
        ...
        for i in range(len(active)):
            for j in range(4):
                v = dm[i, j]
                ax2.text(j, i, f"{v:.1f}", ha="center", va="center", fontsize=5.5,
                         color="white" if v > thresh else "black")
        plt.tight_layout()
        for ext in ("pdf", "png"):
            fn = f"{OUT_PREFIX}_heatmap.{ext}"
            plt.savefig(fn, **({"dpi": 300} if ext == "png" else {}))
    ...
    grp_names = "  ".join(f"{FAULT_LABELS[g].split('–',1)[-1].strip()[:W]:>{W}}"
                          for g in FAULT_GROUPS)
    print(f"  {'Format':<10} {'Clean%':>7}  {'FP32Δ':>6}  {'Status':<12}  {grp_names}")
    print(f"  {'':─<10} {'':─>7}  {'':─>6}  {'':─<12}  " +
          "  ".join("─" * W for _ in FAULT_GROUPS))
    for fmt in ALL_FMTS:
        ca = clean_acc[fmt]
        if fmt in skipped:
            print(f"  {fmt:<10} {ca:>6.2f}%  {fp32_acc-ca:>5.2f}%  "
                  f"{'SKIPPED':<12}  (fault injection not performed)")
        else:
            row = f"  {fmt:<10} {ca:>6.2f}%  {fp32_acc-ca:>5.2f}%  {'OK':<12}  "
            for g in FAULT_GROUPS:
                row += (f"  {ca-results[fmt][g][1]['mean']:>4.1f}/"
                        f"{ca-results[fmt][g][4]['mean']:>4.1f}/"
                        f"{ca-results[fmt][g][8]['mean']:>5.1f}%  ")
            print(row)
    print(sep)

    # ── JSON ──────────────────────────────────────────────────────────────────
    out = {
        "model": MODEL_NAME, "dataset": DATASET_NAME, "num_classes": NUM_CLASSES,
        "ptq_mode": "weight-only", "eval_set": "full_test_10000",
        "n_trials": N_TRIALS, "fp32_baseline": fp32_acc,
        ...
        "formats": {"float16": [k for k in FLOAT_FORMATS if FLOAT_FORMATS[k][2]==16],
                    "float8":  [k for k in FLOAT_FORMATS if FLOAT_FORMATS[k][2]==8],
                    "int":     list(INT_FORMATS.keys())},
        "fault_groups": FAULT_LABELS,
        "clean_acc": clean_acc,
        "results": {
            fmt: {grp: {str(nf): results[fmt][grp][nf] for nf in N_FAULTS}
...
if __name__ == "__main__":
    if DEVICE.type != "cuda":
        print("[WARN] No CUDA GPU – NumPy fallback (very slow with full 10k set).\n")
    print("=" * 72)
